...
def seed_emulators():
    """Seed emulators from JSON file"""
    try:
        with open('seed_data/emulators.json', 'r') as f:
            data = json.load(f)
            
        for emulator in data['emulators']:
            emulator_functions.create_emulator(
                emulator['system_id'],
                emulator['emulator_name'],
                emulator['emulator_file'],
                emulator['emulator_img']
            )
        print("Emulators seeded")
    except FileNotFoundError:
        print("emulators.json not found, skipping...")
    except Exception as e:
        print(f"Error seeding emulators: {e}")
        raise
